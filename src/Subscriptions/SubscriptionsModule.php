...
class SubscriptionsModule {
	...
	private function import_data( array $data ) {
		...
		foreach ( $data as $item ) {
			...
			if ( ! empty( $mollie_customer_id_key ) ) {
				update_user_meta( $customer->get_user_id(), '_pronamic_pay_mollie_customer_id', $item[ $mollie_customer_id_key ] );
	...
	public function maybe_process_import_file_upload() {
		...
		$file = file( $_FILES[ $name ]['tmp_name'], FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES ); // WPCS: input var okay. // WPCS: sanitization ok.
