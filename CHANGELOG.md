...
- Added optional `total_amount` support to `Payment::__construct(...)`.
- Updated payment JSON hydration to pass `total_amount` into `Payment::__construct(...)` so initialized refunded amounts use the correct currency.
